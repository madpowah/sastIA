...
from app.schemas import AuditResponse, AuditListResponse, DashboardStats
...
def create_audit(
    ...
    audit = Audit(
        ...
        docker_status=DockerStatus.SUCCESS if docker_analysis_enabled else DockerStatus.NOT_STARTED,
    )
    db.add(audit)
    db.commit()
    db.refresh(audit)
